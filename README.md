...
### Example: Backup /data/web with rotation

```shell
docker run --pull=always --rm -v /data/web:/data:ro \
  -e ROTATION=1 \
  -e ROTATION_DRY_RUN=1 \
  -e BACKUP_NAME=files \
  -e S3_BUCKET=web-backup \
  -e S3_PATH=data \
  -e S3_NAME_PREFIX=data \
  -e S3_ACCESS_KEY='access_key' \
  -e S3_SECRET_KEY='secret' \
  ssedov/dirbackup-s3cmd:3.0
```

### Example: Backup CIFS \\192.168.1.254\Web-Files

```shell
docker run --pull=always --rm --privileged \
  -e BACKUP_SRC=cifs \
  -e CIFS_HOST=192.168.1.254 \
  -e CIFS_SHARE=Web-Files \
  -e CIFS_USER=cifsuser \
  -e CIFS_PASSWORD=cifspass \
  -e BACKUP_NAME=files \
  -e S3_BUCKET=web-backup \
  -e S3_NAME_PREFIX=data \
  -e S3_ACCESS_KEY='access_key' \
...
- `S3_BUCKET`: Имя бакета в хранилище S3, требуется обязательно.
